...
void calibrarCor(const String &nomeCor)
{
    Serial.print("> Analisando ");
    Serial.print(nomeCor);
    Serial.println(", leitura começa em 5 segundos:");
    delay(TEMPO_ESPERA);
    
    RGB amostras[TOTAL_AMOSTRAS];
    int capturadas = 0;

    while (capturadas < TOTAL_AMOSTRAS)
    {
        RGB atual = lerRGB(S2_DIR, S3_DIR, OUT_DIR);

        bool repetida = false;
        for (int i = 0; i < capturadas; i++)
        {
            if (abs(atual.red - amostras[i].red) <= 1 &&
                abs(atual.green - amostras[i].green) <= 1 &&
                abs(atual.blue - amostras[i].blue) <= 1)
            {
                repetida = true;
                break;
            }
        ...
        if (!repetida)
        {
            amostras[capturadas++] = atual;

            Serial.print("  {");
            Serial.print(atual.red);
            Serial.print(", ");
            Serial.print(atual.green);
            Serial.print(", ");
            Serial.print(atual.blue);
            Serial.print(", \"");
            Serial.print(nomeCor);
            Serial.println("\"},");
        }

        delay(300);  // Aguarda um pouco entre amostras
    }
...
void loop()
{
    ...
    while (true); // Para o loop após calibrar todas
}